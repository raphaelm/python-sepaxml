...
.. code:: python
    ...
    payment = {
        ...
        # Or "structured_description" for structured creditor reference (must not use both)
        # "structured_description": "RF18539007547034",  # ISO 11649 creditor reference
        # "structured_description_type": "ISO",  # Optional: 'ISO' (default) or 'BBA'
...
The library supports two types of payment descriptions: unstructured and structured. You must use either one or the other, not both simultaneously. If neither is provided, an exception will be raised.

**Unstructured descriptions** - Use the ``description`` field for free text descriptions (up to 140 characters) Find an example hereunder:
...
**Structured descriptions** - Use the ``structured_description`` field for standardized creditor references (like ISO 11649 RF or Belgian BBA format). The format type can be specified with ``structured_description_type`` (supports ``'ISO'`` for ISO 11649 or ``'BBA'`` for Belgian format, defaults to ``'ISO'``). Find an example hereunder:
...
.. code:: python
    ...
    payment = {
        # ... other required fields ...
        "structured_description": "RF18539007547034",  # ISO 11649 creditor reference
        "structured_description_type": "ISO",  # Optional, defaults to 'ISO'
    }

For Belgian BBA format:

.. code:: python

    payment = {
        # ... other required fields ...
        "structured_description": "000/0000/00196",  # Belgian BBA format
